...
Do actual files
"""
def doFiles(RUNNumber, seeds, timeEnd, rate, path_to_make, streamName, sizePerFile, ls = 5, theBUNumber = 1):
   ...
   while ((float(timeEnd) < 0.0 or float(time.time() - start) < float(timeEnd)) and (int(LSNumber) == int(ls))):
     ...
     nInput += 100
     nOutput += 9
     fileName =  "%sDATA/Run%d/Data.%d.LS%d.%s.%d.BU%d.raw" % (path_to_make,RUNNumber,RUNNumber,LSNumber,streamName,seedsRND[0],int(theBUNumber))
     ...
     fileNameAUX =  "%sAUX/Run%d/Data.%d.LS%d.%s.BU%d.dat" % (path_to_make,RUNNumber,RUNNumber,LSNumber,streamName,int(theBUNumber))
     msg = "echo %s >> %s" % (fileNameNoDir,fileNameAUX)
     os.system(msg)

     if(theNLoop%50 == 0):
        fileJSONName=  "%sMON/Data.%d.LS%d.%s.BU%d.jsn" % (path_to_make,RUNNumber,LSNumber,streamName,int(theBUNumber))
        fileNameAUX =  "%sAUX/Run%d/Data.%d.LS%d.%s.BU%d.dat" % (path_to_make,RUNNumber,RUNNumber,LSNumber,streamName,int(theBUNumber))
	...
	while(nFileLine != ''):
	   ...
	   listOfFilesDict.append(nFile[0])
           nFileLine = fileAUX.readline()
        fileAUX.close()	   
        outputName = "merged/Run%d/Data.%d.LS%d.%s.BU%d.raw" % (RUNNumber,RUNNumber,LSNumber,streamName,int(theBUNumber))
	...
	theJSONfile.write("\n}\n")
        ...
        fileJSONNameFinal =  "%sMON/Run%d/Data.%d.LS%d.%s.BU%d.jsn" % (path_to_make,RUNNumber,RUNNumber,LSNumber,streamName,int(theBUNumber))
...
def createFiles(streamName = "StreamA", sizePerFile = 50, ls = 10, theBUNumber = 1, path_to_make = "unmerged", rate = 0.0, seeds = 999, timeEnd = -1, RUNNumber = 100):
   ...
   doFiles(int(RUNNumber), seeds, timeEnd, rate, path_to_make, streamName, sizePerFile, ls, theBUNumber)
   return 0
